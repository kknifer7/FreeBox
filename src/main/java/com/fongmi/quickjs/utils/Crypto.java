...
public class Crypto {
    ...
    public static String aes(String mode, boolean encrypt, String input, boolean inBase64, String key, String iv, boolean outBase64) {
        try {
            ...
            byte[] inBuf = inBase64 ? Base64.decode(input.replaceAll("_", "/").replaceAll("-", "+")) : input.getBytes(StandardCharsets.UTF_8);
            return outBase64 ? Base64.encode(cipher.doFinal(inBuf)) : new String(cipher.doFinal(inBuf), StandardCharsets.UTF_8);
        } catch (Exception e) {
            log.error("aes failed", e);
            return "";
        }
    }
    ...
    public static String rsa(String mode, boolean pub, boolean encrypt, String input, boolean inBase64, String key, boolean outBase64) {
        try {
            ...
            byte[] inBytes = inBase64 ? Base64.decode(input.replaceAll("_", "/").replaceAll("-", "+")) : input.getBytes(StandardCharsets.UTF_8);
            ...
            return outBase64 ? Base64.encode(outBytes) : new String(outBytes, StandardCharsets.UTF_8);
        } catch (Exception e) {
            log.error("rsa failed", e);
            return "";
        }
    }
    ...
    private static Key generateKey(boolean pub, String key) throws Exception {
        ...
        return pub ? KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(Base64.decode(key))) : KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(Base64.decode(key)));
    }
